...
describe("CacheKeys", () => {
  ...
  it("should generate correct user timeline cache key", () => {
    expect(CacheKeys.userTimeline("user123", 1)).toBe(
      "user_timeline_user123_1",
    );
    expect(CacheKeys.userTimeline("user456", 3)).toBe(
      "user_timeline_user456_3",
    );
  });

  it("should generate correct user tracks cache key", () => {
    expect(CacheKeys.userTracks("user123", "category456", 1)).toBe(
      "user_tracks_user123_category456_1",
    );
    expect(CacheKeys.userTracks(undefined, undefined, 2)).toBe(
      "user_tracks_all_all_2",
    );
  });

  it("should generate correct search tracks cache key", () => {
    expect(CacheKeys.searchTracks("test query")).toBe(
      "search_tracks_test query",
    );
  });
